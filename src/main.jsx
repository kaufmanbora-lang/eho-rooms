...
function App() {
  ...
  const [callVolume, setCallVolume] = useState(180);
  ...
  function startAudioMeterLoop() {
    ...
    const tick = () => {
      ...
      const speaking = controlsRef.current.mic && localLevel > 0.12;
      const now = Date.now();
...
function ParticipantCard({ person, muted, voiceLevel, index, you }) {
  const activeLevel = muted ? 0 : Math.max(voiceLevel, person.speaking ? 0.42 : 0);
  const speaking = activeLevel > 0.08;
  const ring = speaking ? 86 + activeLevel * 10 : 58 + index * 4;
  return (
  ...
  return (
    <div className={`voice-bars ${muted ? 'muted' : ''}`} aria-label="Voice level">
      {bars.map((weight, index) => (
        <span
          key={index}
          style={{ '--bar': `${Math.max(14, Math.round(16 + level * weight * 44))}%` }}
        />
      ))}
    </div>
  );
...
function AudioSink({ stream, muted, volume }) {
  ...
  useEffect(() => {
    if (gainRef.current) {
      gainRef.current.gain.value = Math.max(0, volume / 100);
    }
    if (ref.current) {
