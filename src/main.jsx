...
function App() {
  ...
  function enhanceOpusSdp(sdp) {
    ...
    const wanted = ['minptime=10', 'useinbandfec=1', 'stereo=0', 'sprop-stereo=0', 'maxaveragebitrate=128000', 'cbr=1'];
    const fmtpIndex = lines.findIndex((line) => line.startsWith(`a=fmtp:${payloadType}`));
    if (fmtpIndex >= 0) {
      const existing = lines[fmtpIndex];
      const missing = wanted.filter((item) => !existing.includes(item.split('=')[0]));
      if (missing.length) lines[fmtpIndex] = `${existing};${missing.join(';')}`;
    } else {
...
function remoteGainValue(volume) {
  ...
  return Math.min(5.5, normalized * 1.05);
}
