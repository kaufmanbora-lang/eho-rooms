...
function App() {
  ...
  function prepareMicrophoneTrack(inputStream) {
    ...
    const cleaned = createCleanVoiceTrack(rawTrack);
    if (!cleaned) {
      micCleanupRef.current = () => {
  ...
  function createCleanVoiceTrack(rawTrack) {
    ...
    try {
      ...
      const updateGate = () => {
        ...
        if (rms > 0.018) openUntil = now + 190;
        const quietTarget = rms < 0.007 && now > openUntil ? 0.06 : 0.22;
        const target = now < openUntil ? 1 : quietTarget;
        gate.gain.setTargetAtTime(target, context.currentTime, target === 1 ? 0.018 : 0.055);
        animationFrame = requestAnimationFrame(updateGate);
      };
  ...
  function enhanceOpusSdp(sdp) {
    ...
    return preferAudioBandwidth(lines, 256000).join('\r\n');
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const updateGate = () => {
        ...
        const rms = Math.sqrt(sum / data.length);
        const now = performance.now();
        if (rms > 0.014) openUntil = now + 170;
        const target = now < openUntil ? 1 : 0.1;
        gate.gain.setTargetAtTime(target, context.currentTime, target === 1 ? 0.015 : 0.06);
        animationFrame = requestAnimationFrame(updateGate);
      };
      animationFrame = requestAnimationFrame(updateGate);
    } catch {
