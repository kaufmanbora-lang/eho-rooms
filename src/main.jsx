...
function App() {
  ...
  async function ensureLocalStream() {
    ...
    if (!hasLiveAudio) {
      const audioStream = await navigator.mediaDevices.getUserMedia({
        audio: audioCaptureConstraints({ aiDenoise: controlsRef.current.noise }),
        video: false
      });
      const audioTrack = await prepareMicrophoneTrack(audioStream, controlsRef.current.noise);
  ...
  function audioCaptureConstraints({ aiDenoise = false } = {}) {
    const supported = navigator.mediaDevices?.getSupportedConstraints?.() ?? {};
    return {
      ...(supported.echoCancellation ? { echoCancellation: true } : {}),
      ...(supported.noiseSuppression ? { noiseSuppression: !aiDenoise } : {}),
      ...(supported.autoGainControl ? { autoGainControl: false } : {}),
      ...(supported.voiceIsolation ? { voiceIsolation: !aiDenoise } : {}),
  ...
  function tuneLocalAudioTrack(track) {
    if (track.kind !== 'audio' || typeof track.applyConstraints !== 'function') return;
    track.applyConstraints(audioCaptureConstraints()).catch(() => {});
  }

  async function prepareMicrophoneTrack(inputStream, useAiDenoise) {
    ...
    if (useAiDenoise) {
      const processed = await createAiDenoisedTrack(inputStream, rawTrack);
      if (processed) return processed;
      await rawTrack.applyConstraints?.(audioCaptureConstraints({ aiDenoise: false })).catch(() => {});
    }

    verifyMicProcessing(rawTrack, false);
    micCleanupRef.current = () => {
  ...
  async function createAiDenoisedTrack(inputStream, rawTrack) {
    const AudioContextClass = window.AudioContext || window.webkitAudioContext;
    if (!AudioContextClass) {
      setNotice('AI noise suppression needs AudioWorklet support');
      return null;
    }
    ...
    try {
      ...
      if (!processedTrack) throw new Error('AI noise suppression did not create an audio track.');
      if ('contentHint' in processedTrack) processedTrack.contentHint = 'speech';
      rawTrack.onended = () => processedTrack.stop();
      ...
      setNotice('AI noise suppression is on');
      ...
      destination?.stream?.getTracks?.().forEach((track) => track.stop());
      context?.close?.().catch(() => {});
      setNotice(`AI noise suppression failed, using browser noise suppression: ${error.message || 'not available'}`);
      return null;
    }
  }

  function verifyMicProcessing(track, aiDenoise) {
    if (aiDenoise) return;
    ...
    if ('autoGainControl' in settings && settings.autoGainControl === false) disabled.push('auto gain');
  ...
  function applyAudioSenderQuality(sender) {
    ...
    parameters.encodings[0].maxBitrate = 160000;
    parameters.encodings[0].priority = 'high';
    parameters.encodings[0].networkPriority = 'high';
    if ('dtx' in parameters.encodings[0]) parameters.encodings[0].dtx = true;
  ...
  function enhanceOpusSdp(sdp) {
    ...
    return preferAudioBandwidth(lines, 160000).join('\r\n');
  }
  ...
  async function rebuildMicrophoneTrack(useAiDenoise) {
    const stream = localStreamRef.current;
    ...
    const audioStream = await navigator.mediaDevices.getUserMedia({
      audio: audioCaptureConstraints({ aiDenoise: useAiDenoise }),
      video: false
    });
    const nextTrack = await prepareMicrophoneTrack(audioStream, useAiDenoise);
    nextTrack.enabled = micWasEnabled;
    stream.addTrack(nextTrack);
    await replaceAudioTrackInPeers(nextTrack);
    bumpLocalMedia();
    rebuildAudioMeters();
    setNotice(useAiDenoise ? 'AI noise suppression is on' : 'Browser noise suppression is on');
  }
  ...
  async function toggleControl(key) {
    try {
      ...
      if (key === 'noise') {
        ...
        controlsRef.current = { ...controlsRef.current, noise: enabled };
        await rebuildMicrophoneTrack(enabled);
        return;
      }
  ...
  return (
    <footer className="call-dock">
      ...
      <div className="call-controls">
        ...
        <DockButton label="AI noise suppression" active={controls.noise} onClick={() => onToggle('noise')} icon={WandSparkles} />
...
function remoteGainValue(volume) {
  ...
  return Math.min(2.7, 1 + (normalized - 1) * 0.72);
}
