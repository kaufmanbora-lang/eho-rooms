...
- Browser noise suppression, echo cancellation, and a Studio voice enhance mode for clearer speech with less hiss, rumble, and clipping
...
EchoRoom sends Opus voice at 48 kHz with DTX disabled, browser echo cancellation/noise suppression enabled, and a Studio voice enhance chain for rumble cut, presence, hiss taming, compression, and limiting.

If voice still crackles on Windows, check the selected input/output device. Bluetooth `Hands-Free`, `Handsfree`, or `AG Audio` modes can sound distorted in any browser. Use a normal microphone input and stereo headphones/speakers output for the cleanest sound.
