...
By default, account/channel data is stored in `server/data/store.json`. The included `render.yaml` is configured for Render's free web service tier, so it does not attach a disk. The app will run on Render even when your computer is off, but free-tier file storage can reset after redeploys or restarts. For permanent production data, upgrade the service and add a Render disk, or move the store to a real database.
...
The free Blueprint intentionally does not include `disk:` because Render rejects disks on free services.
