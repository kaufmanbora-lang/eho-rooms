...
app.get('/api/health', (_request, response) => {
  response.json({ ok: true, service: 'echoroom', storage: dbPool ? 'postgres' : 'file' });
});
...
async function initializeStorage() {
  if (!databaseUrl) {
    ...
    console.log(`EchoRoom using file storage at ${storePath}`);
    return;
  }
  ...
  await dbPool.query(`
    CREATE TABLE IF NOT EXISTS echoroom_store (
      id text PRIMARY KEY,
      data jsonb NOT NULL,
  ...
  console.log('EchoRoom using PostgreSQL storage');
}

async function loadStore() {
  if (dbPool) {
    const result = await dbPool.query('SELECT data FROM echoroom_store WHERE id = $1', [storeRecordId]);
    if (result.rows[0]?.data) {
      return normalizeStore(result.rows[0].data);
    }

    const fileCandidate = await readStoreFile();
    const normalized = normalizeStore(fileCandidate);
    if (hasUserData(fileCandidate)) {
      await saveStoreToDatabase(normalized);
      console.log('Migrated existing file store into PostgreSQL');
    }
    return normalized;
  }
...
async function saveStoreToDatabase(nextStore) {
  await dbPool.query(
    `INSERT INTO echoroom_store (id, data, updated_at)
     VALUES ($1, $2::jsonb, now())
     ON CONFLICT (id)
     DO UPDATE SET data = EXCLUDED.data, updated_at = now()`,
    [storeRecordId, JSON.stringify(nextStore)]
  );
}

function hasUserData(candidate) {
  return Boolean(
    Array.isArray(candidate.users) && candidate.users.length
    || Array.isArray(candidate.sessions) && candidate.sessions.length
    || candidate.messages && typeof candidate.messages === 'object' && Object.keys(candidate.messages).length
  );
}

function getDatabaseSslConfig(url) {
  if (/^(1|true|require)$/i.test(process.env.DATABASE_SSL || '')) {
    return { rejectUnauthorized: false };
  }
  if (/^(0|false|disable)$/i.test(process.env.DATABASE_SSL || '')) {
    return false;
  }
